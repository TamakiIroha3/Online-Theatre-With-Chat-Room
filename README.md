# 在线放映室 (Online Theater)

一个基于Python开发的实时视频流分享和聊天室应用，支持SRT/RTMP协议的视频流传输。

## ✨ 功能特性

- 🎬 **视频流分享**：支持通过SRT协议接收OBS推流，并分发给多个观看者
- 💬 **实时聊天室**：内置聊天功能，支持emoji表情
- 🌐 **IPv6支持**：优先支持IPv6网络，同时兼容IPv4
- 👥 **多用户支持**：支持多个接收端同时观看
- 🔒 **验证码保护**：通过6位数字验证码控制访问权限
- 🎮 **角色系统**：使用Fate系列角色名作为默认昵称
- 🔄 **智能重试**：发送端MPV无限循环等待OBS推流
- ⏱️ **延迟启动**：接收端MPV延迟2秒启动，确保流稳定
...
4. **配置OBS推流**：
   - 设置 -> 推流
   - 服务：自定义
   - 服务器：`srt://127.0.0.1:9001` (本地) 或 `srt://[您的IP]:9001` (远程)
   - 串流密钥：留空
...
- GitHub Issues: [提交问题](https://github.com/your-repo/online-theater/issues)
